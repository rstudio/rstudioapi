...
vignette: >
  %\VignetteIndexEntry{Terminal Interaction}
...
Note: In the current implementation, the terminal identifier is the same as the terminal caption. Thus if a terminal's caption is renamed using the RStudio user interface, the terminal identifier will change without the code knowing about it. This limitation will be fixed in a future release, and the identifier and the caption will no longer be the same; don't write code that assumes the identifier and caption are the same thing.
